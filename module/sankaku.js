...
var sankaku = {
    path: config.sankaku.path,
    done: false,
    search: (core, message, searchword, mode, mode2) => {
        if (mode == 'nhentai') {
            if (message.channel.name.indexOf('nsfw') < 0) {
                log.consoleLog(core, c.bgRed('Doujin search at wrong channel.'));
                message.channel.send('retard, itt nem kereshetsz.');
            } else {
                sankaku.nhentaiSearch(core, message, searchword);
            }
        } else if (mode == 'sankaku') {
            if (message.channel.name.indexOf('nsfw') < 0) {
                log.consoleLog(core, c.bgRed('Sankaku search at wrong channel.'));
                message.channel.send('retard, itt nem kereshetsz.');
            } else {
                sankaku.sankakuSearch(core, message, searchword, () => { });
            }
        } else if (mode == 'waifucloud') {
            if (message.channel.name.indexOf('nsfw') < 0) {
                log.consoleLog(core, c.bgRed('WaifuCloud search at wrong channel.'));
                message.channel.send('retard, itt nem kereshetsz.');
            } else {
                waifucloud.search_tags(core, "random", searchword, message);
            }
        }
    },
    httpGet: (url, filename) => {
        var downloadedfile = fs.createWriteStream(sankaku.path + filename);
        sankaku.done = false;
    ...
    sankakuSearch: (core, message, searchword, callback) => {
        ...
        var curl = exec(`curl -s -b ${sankaku.path}cookies.txt "${url_link}" > "${sankaku.path}${message.fname1}"`, (err, stdout, stderr) => {
            if (err) {
                console.log(err);
        ...
        curl.on('exit', (code) => {
            if (!fs.existsSync(sankaku.path + message.fname1)) {
                console.log('File has been not downloaded.1');
                message.channel.send('Valamiért nem jött le a html~');
                return callback();
            }
            var text = fs.readFileSync(sankaku.path + message.fname1).toString().split('\n');
            var postlist = [];
            for (i in text) {
            ...
            var curl2 = exec(`curl -s -b ${sankaku.path}cookies.txt "${post_url}" > "${sankaku.path}${message.fname2}"`, (err, stdout, stderr) => {
                if (err) {
                    console.log(err);
            ...
            curl2.on('exit', (code) => {
                if (!fs.existsSync(sankaku.path + message.fname2)) {
                    console.log('File has been not downloaded.2');
                    message.channel.send('Valamiért nem jött le a html~');
                    return callback();
                }
                var text = fs.readFileSync(sankaku.path + message.fname2).toString().split('\n');
                ...
                var curl3 = exec(`curl -s -b ${sankaku.path}cookies.txt "${original_url}" > "${sankaku.path}${message.fname3}"`, (err, stdout, stderr) => {
                    if (err) {
                        console.log(err);
                ...
                var teszt = false;
                curl3.on('exit', (code) => {
                    if (!fs.existsSync(sankaku.path + message.fname3)) {
                    ...
                    if (!teszt) {
                        teszt = true;
                        webpconvert.file(sankaku.path + message.fname3, (image) => {
                            console.log(image);
                            core.discord.bot.channels.get(core.discord.ch.gekkylog).send({ files: [image] }).then(response => {
                                message.channel.send({
                                    embed: {
                                        ...
                                        "url": original_url,
                                        "color": sankaku.ratingColor(rating)
                                    }
                                });
                            });
                        });
    ...
    nhentaiSearch: (core, message, searchword) => {
        ...
        url_link = url_link + searchword;
        sankaku.httpGet(url_link, ncounter + '.html');
        var inta = setInterval(() => {
            if (sankaku.done) {
                clearInterval(inta);
                if (fs.existsSync(sankaku.path + ncounter + '.html')) {
                    var input = fs.createReadStream(sankaku.path + ncounter + '.html');
                    var array = fs.readFileSync(sankaku.path + ncounter + '.html').toString().split('\n');
                    var no_match = false;
                    var word = '';
                    var i = 0;
                    ...
                    if (!no_match) {
                        ...
                        if (i < array.length) {
                            ...
                            ncounter += 'b';
                            sankaku.httpGet(url_link + '&page=' + page, ncounter + '.html');
                            var inte = setInterval(() => {
                                if (sankaku.done) {
                                    ...
                                    if (doujinlist[rnumber] != undefined) {
                                        ...
                                        log.consoleLog(core, doujinlist[rnumber]);
                                    } else {
                                        message.channel.send('Nincs találat. ¯\\_(ツ)_/¯');
                                        log.consoleLog(core, 'Nincs találat. ¯\\_(ツ)_/¯');
                                    }
                                }
                            }, 100);
                        }
